...
function softenConjunctions(text) {
    const patterns = [
        [/\bFirstly\b,/gi, 'First,'],
...
function diversifyVocabulary(text) {
    const vocabMap = [
...
function hedgeAbsoluteStatements(text) {
    const hedgeMap = [
...
function generateAssembledFrontmatter(fragments, assembledBody) {
    ...
    const fm = { title, description, tags, sources: fragments.length };
    if (earliestDate)
...
function editDocs(dir) {
    ...
    for (const name of entryNames) {
        ...
        const fmObj = { title, description, tags: tags.length > 0 ? tags : [], depends_on: depends.length > 0 ? depends : [], source: name };
...
function linkUp(dir) {
    ...
    for (const f of fragments) {
        ...
        if (newContent !== f.content)
            changed = true;
        const headingRegex = /^(#{1,6})\s+(.+)$/gm;
        newContent = newContent.replace(headingRegex, (match, level, text) => {
...
function ingest(rawFile, targetDir) {
    ...
    const fmObj = { title, description, tags, depends_on: [], source: path.basename(rawFile), status: 'draft' };
    const fmYaml = yaml.dump(fmObj);
    ...
    for (const frag of fragments) {
        if (!frag.raw) {
            ...
            if (!frag.metadata.source)
                entries.push({ file: frag.fileName, severity: 'info', message: 'No source field — consider adding the original document path' });
...
function main() {
    ...
    if (isGraphMode) {
        ...
        if (!targetDir) {
            console.error('Error: directory required for graph mode');
            process.exit(1);
        }
        ...
        if (graphMode) {
            ...
            if (jsonOnly)
                ...
                if (orphans.length === 0)
                    console.log('No orphan documents in ' + targetDir);
                else {
            ...
            if (jsonOnly)
                console.log(JSON.stringify({ target: backlinksTarget, backlinks: backlinks.map(b => ({ file: b.file, fileName: b.fileName, metadata: b.metadata })), count: backlinks.length, durationMs: Date.now() - startTime }, null, 2));
            else {
                if (backlinks.length === 0)
                    console.log('No backlinks to "' + backlinksTarget + '" in ' + targetDir);
                else {
    ...
    if (assembleMode) {
        ...
        if (fragments.length === 0) {
            console.error('Error: no .md files found in ' + targetDir);
            process.exit(1);
        }
        ...
        if (dryRun) {
            ...
            if (jsonOnly) {
                ...
                if (totalChanges > 0)
                    ...
                    console.log('Use --voice to humanize the assembled output');
    ...
    if (editDocsMode) {
        const targetDir = getPositionalArg(2);
        if (!targetDir) {
            console.error('Error: directory required for --edit-docs');
            process.exit(1);
        }
        if (!fs.existsSync(targetDir) || !fs.statSync(targetDir).isDirectory()) {
            console.error('Error: directory not found: ' + targetDir);
    ...
    if (updateIndexMode) {
        const targetDir = getPositionalArg(2);
        if (!targetDir) {
            console.error('Error: directory required for --update-index');
            process.exit(1);
        }
        if (!fs.existsSync(targetDir) || !fs.statSync(targetDir).isDirectory()) {
            console.error('Error: directory not found: ' + targetDir);
    ...
    if (linkUpMode) {
        const targetDir = getPositionalArg(2);
        if (!targetDir) {
            console.error('Error: directory required for --link-up');
            process.exit(1);
        }
        if (!fs.existsSync(targetDir) || !fs.statSync(targetDir).isDirectory()) {
            console.error('Error: directory not found: ' + targetDir);
        ...
        if (jsonOnly) {
            ...
            const parts = ['Links rewired: ' + result.linksRewired, 'Headings deduped: ' + result.headingsDeduped, 'Deps auto-wired: ' + result.depsAutoWired, 'Files changed: ' + result.filesChanged];
    ...
    if (gatherMode) {
        const targetDir = getPositionalArg(2);
        if (!targetDir) {
            console.error('Error: directory required for --gather');
            process.exit(1);
        }
        if (!fs.existsSync(targetDir) || !fs.statSync(targetDir).isDirectory()) {
            console.error('Error: directory not found: ' + targetDir);
    ...
    if (ingestMode) {
        ...
        if (!result) {
            console.error('Error: failed to ingest ' + rawFile);
            process.exit(1);
        }
        if (jsonOnly) {
    ...
    if (!fs.existsSync(cliFile)) {
        const result = { file: cliFile, fileName: path.basename(cliFile, '.md'), voice,
    ...
    if (dryRun) {
        ...
        if (jsonOnly)
            ...
            console.log(`\nFabricated ${cliFile} (voice: ${voice})`);
            console.log(renderSummaryTable(summary));
        ...
        if (jsonOnly)
            ...
            console.log(`\nFabrication analysis for ${cliFile} (voice: ${voice})`);
            console.log(renderSummaryTable(summary));
...
main();
//# sourceMappingURL=md-fabrication.js.map
